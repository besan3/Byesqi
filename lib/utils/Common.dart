...
extension SExt on String {
  ...
  bool get isVideoPlayerFile => this.contains("mp4") || this.contains("m4v") || this.contains("mkv") || this.contains("MOV") || this.contains("mov");
...
Future<void> getDetails({required LoginResponse logRes, required String image}) async {
  await setValue(USER_ID, logRes.userId);
  await setValue(NAME, logRes.firstName);
  await setValue(LAST_NAME, logRes.lastName);
  await setValue(USER_PROFILE, logRes.profileImage.validate().isEmpty ? image : logRes.profileImage.validate());
  await setValue(USER_EMAIL, logRes.userEmail);
  ...
  await Future.forEach(logRes.plan!.subscriptions.validate(), (dynamic data) async {
    ...
    if (data.status == userPlanStatus) {
      await setValue(SUBSCRIPTION_PLAN_ID, data.subscriptionPlanId);
      await setValue(SUBSCRIPTION_PLAN_START_DATE, data.startDate);
      await setValue(SUBSCRIPTION_PLAN_EXP_DATE, data.expirationDate);
      await setValue(SUBSCRIPTION_PLAN_TRIAL_STATUS, data.trailStatus);
      await setValue(SUBSCRIPTION_PLAN_NAME, data.subscriptionPlanName);
      await setValue(SUBSCRIPTION_PLAN_AMOUNT, data.billingAmount);
      await setValue(SUBSCRIPTION_PLAN_TRIAL_END_DATE, data.trialEnd);

      appStore.setSubscriptionPlanId(data.subscriptionPlanId);
      appStore.setSubscriptionPlanStartDate(data.startDate);
      appStore.setSubscriptionPlanExpDate(data.expirationDate);
      appStore.setSubscriptionPlanStatus(data.status);
      appStore.setSubscriptionPlanName(data.subscriptionPlanName);
      appStore.setSubscriptionPlanAmount(data.billingAmount);
      appStore.setSubscriptionTrialPlanStatus(data.trailStatus);
      appStore.setSubscriptionTrialPlanEndDate(data.trialEnd);
    }
  });
}
      ...
      for (var i in list) {
        if (i.id == data.id) {
          ...
          await prepareSaveDir().then((value) {
            final savedDir = File(data.filePath.validate());
...
void shareMovieOrEpisode(String videUrl) {
  Share.share(videUrl);
}
