...
import 'package:streamit_flutter/screens/download_file/DownloadFileScreen.dart';
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          preferredSize: movie.isPostRestricted.validate() || appStore.isTrailerVideoPlaying ? Size(context.width(), kToolbarHeight) : Size(0,60),
          child: appBarWidget(
                        ...
                        Row(
                          children: [
                            Column(
                              ...
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                headingText(parseHtmlString(movie.title.validate()), fontSize: 25, maxLines: 2),
                                8.height,
                                  ...
                                  children: [
                                    ClipRRect(
                                      ...
                                      child: BackdropFilter(
                                        ...
                                        //   child: Icon(Icons.share_rounded, color: colorPrimary, size: 18),
                                        // ),
                                      ),
                                    ).onTap(() {
                                      shareMovieOrEpisode(widget.movieData.shareUrl.validate());
                                    }).paddingRight(8),
                                    if (movie.file.validate().isNotEmpty && !movie.isPostRestricted.validate())
                                      ClipRRect(
                                        borderRadius: BorderRadius.circular(50),
                                        child: BackdropFilter(
                                          filter: ImageFilter.blur(sigmaX: 5, sigmaY: 5),
                                          child: Container(
                                            padding: EdgeInsets.all(12),
                                            color: colorPrimary.withOpacity(0.2),
                                            child: Icon(Icons.cast_rounded, color: colorPrimary, size: 18),
                                          ),
                                        ),
                                      ).onTap(() {
                                        VideoCastDeviceListScreen(
                                          videoURL: widget.movieData.urlLink.validate(),
                                          videoTitle: widget.movieData.title.validate(),
                                          videoImage: widget.movieData.attachment.validate(),
                                        ).launch(context);
                                      }).paddingRight(8),
                                    if (movie.file.validate().isNotEmpty && appStore.isLogging && !movie.isPostRestricted.validate())
