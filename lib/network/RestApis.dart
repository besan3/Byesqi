...
Future<LoginResponse> token(Map request) async {
  ...
  appStore.setUserProfile(res.profileImage.validate().isEmpty ? userProfile : res.profileImage.validate());
...
Future<List<MovieData>> searchMovie(String aSearchText, {int page = 1, required List<MovieData> movies, bool isLoading = true}) async {
  appStore.setLoading(isLoading);

  try {
    var res = MovieResponse.fromJson(await (handleResponse(await getRequest('streamit-api/api/v1/streamit/search-list?search=$aSearchText&user_id=$mUserId&paged=$page&posts_per_page=$postPerPage'))));
...
Future<void> saveVideoContinueWatch({required ResumeVideoModel videoData}) async {
  await handleResponse(await postRequest('streamit-api/api/v1/streamit/save-continue-watch', body: videoData.toJson(), aAuthRequired: true));
}

Future<Map<String, dynamic>> getVideoContinueWatch() async {
  return await handleResponse(await getRequest('streamit-api/api/v1/streamit/get-continue-watch', aAuthRequired: true));
}
